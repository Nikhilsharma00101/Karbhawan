...
interface EditableProduct {
    ...
    compatibility?: { make: string; model: string; years?: string[] }[];
    isUniversal?: boolean;
    installationOverride?: { isAvailable: boolean; flatRate?: number; segmentRates?: Record<string, number> };
}
...
export default function EditProductForm({ product }: { product: EditableProduct }) {
    ...
    const [state, formAction] = useActionState(updateProductWithId, { message: "", success: false });

    // --- STATE INITIALIZATION FROM PRODUCT PROP ---
    ...
    const [selectedSubSubCategory, setSelectedSubSubCategory] = useState(product.subSubCategory || "");
    ...
    const [isUniversal, setIsUniversal] = useState(product.isUniversal || false);
    const [compatibilities, setCompatibilities] = useState<{ make: string, model: string }[]>(product.compatibility || []);
    ...
    return (
        <div className="max-w-7xl mx-auto pb-24 px-4 sm:px-6 lg:px-8">
            <header className="mb-16 space-y-2 text-center md:text-left">
                <h2 className="text-5xl heading-luxe text-aether-primary">Edit Product</h2>
                <p className="text-aether-secondary font-medium italic">Update product details for #{product._id.toString().slice(-6).toUpperCase()}</p>
            </header>

            <form action={formAction}>
                {/* Top Actions for visibility - Optional but useful for long form */}
                <div className="flex justify-end gap-4 mb-6">
                    <button type="button" onClick={() => window.history.back()} className="px-6 py-3 rounded-xl border border-slate-200 text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">
                        Cancel
                    </button>
                    <SubmitButton />
                </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input name="price" type="number" required min="0" defaultValue={product.price} className={`${inputClasses} pl-8 font-mono`} placeholder="0.00" />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="relative">
                                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-serif italic">₹</span>
                                            <input name="discountPrice" type="number" min="0" defaultValue={product.discountPrice} className={`${inputClasses} pl-8 font-mono`} placeholder="Optional" />
                                        </div>
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input name="stock" type="number" required min="0" defaultValue={product.stock} className={inputClasses} placeholder="Available units" />
                                    </div>
                                    <div>
                                        <label className={labelClasses}>SKU (Stock Keeping Unit)</label>
                                        <input name="sku" type="text" defaultValue={product.sku} className={`${inputClasses} font-mono uppercase`} placeholder="PROD-001" />
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                        <input
                                            ...
                                            defaultChecked={product.installationOverride?.isAvailable ?? true}
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            type="number"
                                            min="0"
                                            defaultValue={product.installationOverride?.flatRate}
                        ...
                        <div className="aether-card p-6 space-y-6 border-slate-200/60 shadow-lg shadow-slate-200/50">
                            ...
                            <div className="space-y-3 pt-2">
                                <div className="flex justify-between items-center p-3 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer group">
                                    ...
                                    <input type="checkbox" name="featured" defaultChecked={product.featured} className="w-5 h-5 rounded border-slate-300 accent-cta-blue" />
                                </div>
                                <div className="flex justify-between items-center p-3 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer group">
                                    <span className="text-sm font-bold text-slate-700">New Arrival</span>
                                    <input type="checkbox" name="isNewArrival" defaultChecked={product.isNewArrival} className="w-5 h-5 rounded border-slate-300 accent-cta-blue" />
                                </div>
                                <div className="flex justify-between items-center p-3 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer group">
                                    <span className="text-sm font-bold text-slate-700">Best Seller</span>
                                    <input type="checkbox" name="isBestSeller" defaultChecked={product.isBestSeller} className="w-5 h-5 rounded border-slate-300 accent-cta-blue" />
                                </div>
                            </div>
                        ...
                        <div className="aether-card p-6 space-y-6">
                            ...
                            <div className="pt-4 border-t border-slate-100 space-y-4">
                                <div>
                                    ...
                                    <input name="tags" type="text" defaultValue={product.tags?.join(", ")} className={inputClasses} placeholder="e.g. sporty, sleek..." />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className={labelClasses}>Colors</label>
                                        <input name="colors" type="text" defaultValue={product.colors?.join(", ")} className={inputClasses} placeholder="Black, Red..." />
                                    </div>
                                    <div>
                                        <label className={labelClasses}>Sizes</label>
                                        <input name="sizes" type="text" defaultValue={product.sizes?.join(", ")} className={inputClasses} placeholder="S, M, L..." />
                                    </div>
                                </div>
                            </div>
                        </div>
